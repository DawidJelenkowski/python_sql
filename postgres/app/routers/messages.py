...
from app.dependencies import get_db, validate_user
...
def write_a_message_on_the_guestbook(message: str = Form(...),
...
def update_a_specific_message(message_id: int,
                              message: str = Form(...),
                              private: bool = Form(False),
                              db: Database = Depends(get_db),
                              user_id: str = Depends(validate_user)):
    
    message_db = db.get_one('guestbook', 
                            ['id', 'user_id'], 
                            where={'id': message_id})
    ...
    raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                        detail="You are not allowaed to update this message")


@router.get('/messages/{message_id}')
def get_a_specified_message(message_id: int,
                            db: Database = Depends(get_db),
                            user_id: int = Depends(validate_user)):
...
@router.get('/messages')
def get_all_messages(num: int = 10,
                    db: Database = Depends(get_db),
                    user_id: str = Depends(validate_user)):
